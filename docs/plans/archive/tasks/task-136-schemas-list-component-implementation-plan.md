# Task #136: Create SchemasList Component with SchemaCard Items

**Plan Task:** #136
**Wave/Phase:** Phase 2 Settings & Management UI - Custom Fields System
**Dependencies:** Task #118 (useSchemas hook), Task #68 (Field Schemas CRUD endpoints)

---

## 🎯 Goal

Create a responsive SchemasList component that displays all field schemas as individual SchemaCard items in a grid layout. Each card shows schema metadata (name, description, field count, tag usage) and provides actions (Edit, Delete, Duplicate) via a dropdown menu. The component integrates with the useSchemas hook from Task #118 and follows the established VideoCard/VideoGrid pattern.

**Expected Outcome:** Production-ready SchemasList and SchemaCard components with full TypeScript support, comprehensive test coverage (25+ tests), shadcn/ui styling, WCAG 2.1 Level AA accessibility compliance, and seamless integration with SettingsPage (Task #135).
...
- [ ] **SchemasList Component**
  ...
  - [ ] Fetches schemas via useSchemas hook (from Task #118)
...
- Required for SchemasList component (Task #136)
...
export function SchemasList({
   ...
   *
   * TODO (Task #137): Implement actual tag count query
  ...
  const getTagCount = (schemaId: string): number => {
    // Placeholder - real implementation in Task #137
    return 0
  }
...
3. **Placeholder Tag Count:** Documents future Task #137 implementation
...
- Renders SchemaCard for each schema
- Placeholder tag count calculation (Task #137)
- Handles empty state gracefully
...
- `SchemasList.tsx` - Responsive grid of schema cards (Task #136)
  - Grid layout: 1 column (mobile), 2 columns (tablet), 3 columns (desktop)
  - Renders SchemaCard for each schema
  - Placeholder tag count calculation (real implementation in Task #137)
- `SchemaCard.tsx` - Individual schema card with metadata and actions (Task #136)
...
- Document placeholder tag count (Task #137)
...
**Decision:** Return 0 for all schemas until Task #137
...
**Validated:** useSchemas hook from Task #118 already implements queryOptions factory pattern

**Evidence:**
```typescript
// Task #118 implementation
...
- Task #118: useSchemas React Query hook - COMPLETE

**Blocks:**
- Task #137: Add schema actions (Edit, Delete, Duplicate handlers)
- Task #138: Implement tag usage count query
- Task #135: SettingsPage integration (SchemasList rendering)
...
**Phase 1 - Immediate (Post-Task #136):**
- Task #137: Implement action handlers (Edit opens SchemaEditor, Delete shows confirmation, Duplicate creates copy)
- Task #138: Replace placeholder tag count with real query

**Phase 2 - Settings Page (After Task #135):**
...
- `docs/plans/tasks/task-118-use-schemas-hook.md` - useSchemas implementation
