# Task #139: Add Field Actions (Edit, Delete, Show Usage Count)

**Status:** Planning Complete  
**Dependencies:** Task #138 (FieldsList component), Tasks #58-#69 (Backend endpoints)  
...
**File:** `frontend/src/components/settings/FieldsList.tsx` (created in Task #138)
      ...
      <TableBody>
        {fields.map((field) => (
          <TableRow key={field.id}>
            ...
            <TableCell>{/* Config display logic from Task #138 */}</TableCell>
            {showUsageCount && (
 ...
 * Full visual config editor can be added later (Task #140).
 *
 * Validation:
...
**Future Enhancement (Task #140):** Replace JSON textarea with visual config editors per field type (rating slider, select options multi-input)
...
import { useSchemas } from './useSchemas' // Assumes Task #138 created this hook
...
**Create file:** `docs/plans/tasks/task-139-manual-testing.md`

```markdown
# Task #139 Manual Testing Checklist
...
**File:** `task-139-manual-testing.md` - 16 test cases
...
**Task Dependencies:**
- Task #138: FieldsList component (must exist before adding actions)
...
- [ ] Task #139 marked complete in status.md
...
**Next Task:** Task #140 (Visual config editors) - OPTIONAL enhancement
