# Task #150: Add get_current_user dependency

**Plan Task:** #150
**Wave/Phase:** Wave 3 Security / JWT Authentication System
**Dependencies:** Task #149 (Security utilities - password hashing, JWT functions)
...
**Key Points:**
- `tokenUrl="/api/auth/login"` matches the endpoint that will be created in Task #151
...
**Changes:**
- Import `ALGORITHM` from `app.core.security` (created in Task #149)
...
**If tests fail:**
1. Check that Task #149 is complete (`app.core.security` module exists)
...
Dependencies are ready for use in protected endpoints (Task #152).

Ref: Task #150
Related: Task #149 (security utilities), Task #151 (auth endpoints)
...
- Create tokens with `create_access_token` from Task #149
...
**Test with curl (after Task #151 login endpoint is complete):**

```bash
# 1. Login to get token (requires Task #151)
...
# 2. Test protected endpoint (requires Task #152)
curl http://localhost:8000/api/lists \
  -H "Authorization: Bearer eyJ..."

# Should return lists (after endpoints are protected in Task #152)
```
...
**Dependencies:**
- Task #149: `app.core.security` module with `ALGORITHM` constant and `create_access_token` function
- Task #151: Login endpoint that will use these dependencies
- Task #152: Protect existing endpoints with `get_current_user`
...
**Prerequisites:**
- Task #149 must be complete (security utilities with `create_access_token` and `ALGORITHM`)
- Database must be running for integration tests
- Test user fixtures available in `conftest.py`

**Blockers:**
- None (all dependencies from Task #149)
...
- [ ] Ready for Task #151 (login endpoint) and Task #152 (protect endpoints)
