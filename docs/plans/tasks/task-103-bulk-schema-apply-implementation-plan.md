# Task #103: Add Bulk Operations (Apply Schema to Multiple Tags) - Implementation Plan

**Created:** 2025-11-08  
**Status:** Planning Complete - Ready for Execution  
**Phase:** Phase 2 - Settings & Management UI (Custom Fields System)  
**Dependencies:** Task #97 (SettingsPage), Task #98 (SchemasList), Task #99 (Schema Actions)  
...
```
SettingsPage (Task #97)
└── SchemasList (Task #98)
    └── SchemaCard (Task #98)
        └── SchemaActionsMenu (Task #99)
            └── BulkApplySchemaDialog (NEW - Task #103)
...
```
1. User clicks "Apply to Tags" in SchemaCard menu (Task #99)
...
**File:** `frontend/src/components/SchemaCard.tsx` (MODIFY from Task #99)
...
```typescript
// frontend/src/components/SchemaCard.tsx (additions to Task #99 implementation)
...
export function SchemaCard({ schema, listId, ...otherProps }: SchemaCardProps) {
  ...
  return (
    <>
      <Card>
        {/* Existing SchemaCard UI from Task #98/#99 */}
        <CardHeader>
...
- [ ] No regressions in existing tests (Task #97-#99)
...
**Depends On:**
- Task #97: SettingsPage (provides route `/settings/schemas`) - COMPLETE
- Task #98: SchemasList/SchemaCard (provides SchemaCard component) - COMPLETE
- Task #99: Schema Actions (provides action menu in SchemaCard) - COMPLETE

**Blocks:**
- None (Task #103 is final in Phase 2)

**Related:**
- Task #70: Tag Update Endpoint (backend API used by bulk operation) - COMPLETE
- Task #102: Schema Templates (may benefit from bulk apply for template application)
...
- `docs/plans/tasks/task-099-schema-actions-implementation-plan.md` - Schema actions (prerequisite)
...
**End of Implementation Plan - Task #103**
