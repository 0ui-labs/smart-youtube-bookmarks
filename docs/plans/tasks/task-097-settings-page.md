# Task #97: Create SettingsPage Component with /settings/schemas Route

**Goal:** Create a Settings page with tabbed interface for managing schemas and custom fields, following existing UI patterns from VideosPage and Dashboard. Implement the Schemas tab with schema management UI (edit, delete, duplicate, usage stats).

**Architecture:** React page component with React Router v6 integration, shadcn/ui Tabs for navigation between Schemas/Fields sections, TanStack Query for data fetching, and reusable Card-based UI pattern following JobProgressCard style.

**Tech Stack:** React, TypeScript, React Router v6, shadcn/ui (Tabs, Card, Button, DropdownMenu), TanStack Query (useSchemas hook), Vitest for testing.
...
- [ ] **Navigation integration**: Add "Settings" link to VideosPage header or sidebar
...
- Required for SettingsPage UI (Task #97)
...
# Expected: 8 failing tests (component not yet implemented)
```
 ...
 * - Schemas: Manage field schema templates (Task #97)
...
export function SettingsPage() {
  ...
  // TODO: Replace hardcoded listId with actual list context (Task #98)
  const LIST_ID = 'first-available' // MVP single-list approach
  const { data: schemas, isLoading, isError } = useSchemas(LIST_ID)

  // Placeholder handlers (to be implemented in future tasks)
  const handleCreateSchema = () => {
    console.log('Create schema clicked - to be implemented')
    // TODO: Open SchemaEditor dialog (Task #83)
  }
...
- Uses hardcoded listId for MVP (matches App.tsx pattern)
...
export function SchemasList({
  ...
  // TODO: Fetch tag counts for each schema (Task #98)
...
- Placeholder tag count logic (Task #98)
...
- No loading guard needed (SettingsPage handles loading internally)
...
- `SettingsPage.tsx` - Schema and field management (Task #97)
...
**Test Coverage:**
- Unit tests: SettingsPage (8 tests), SchemaCard (11 tests), SchemasList (4 tests)
- Hook tests: useSchemas (3 tests)
- Integration tests: SettingsPage flow (4 tests)
- **Total: 30 tests**
...
- [ ] Verify loading state while schemas fetch
...
**Future Tasks:**
- Task #98: Implement tag usage count query
- Task #99: Implement schema editor dialog
- Task #100: Implement schema delete confirmation
- Task #101: Implement schema duplication logic
