# Task #156: Secure Default Credentials - Add Secret Validation to Config Class

**Plan Task:** #156
**Wave/Phase:** Phase 4 - Security Hardening (Task 2 from Security Plan)
**Dependencies:** Task #154 (secrets generation script) ✅, Task #155 (.env.example with placeholders) ✅
...
- [ ] **Secret Key Validation** (existing validator extended)
  ...
  - [ ] NEW: Also reject "CHANGE_ME_RUN_scripts_generate_secrets_py" (Task #154 placeholder)
...
- [ ] **Documentation**
  ...
  - [ ] Error messages reference Task #154 script: `scripts/generate_secrets.py`
...
def validate_secret_key(cls, v: str, info) -> str:
    ...
    # List of known weak/default values (from Tasks #154-117)
    WEAK_DEFAULTS = [
        "your-secret-key-here-change-in-production",  # Old default
        "CHANGE_ME_RUN_scripts_generate_secrets_py",   # Task #154 placeholder
    ]
...
def test_secret_key_task116_placeholder_rejected_production():
    """Task #154 placeholder secret key in production raises error"""
    with pytest.raises(ValidationError) as exc_info:
        Settings(
            env="production",
...
def test_postgres_password_task116_placeholder_rejected_production():
    """Task #154 placeholder PostgreSQL password in production raises error"""
    with pytest.raises(ValidationError) as exc_info:
        Settings(
            env="production",
            secret_key="a" * 32,
...
def test_redis_password_default_rejected_production():
    """Task #154 placeholder Redis password in production raises error"""
    with pytest.raises(ValidationError) as exc_info:
        Settings(
            env="production",
            secret_key="a" * 32,
...
4. **Reference:** Points to Task #154 script
...
### Complete Settings Class (After Task #156)
...
**Depends On:**
- Task #154: `scripts/generate_secrets.py` ✅
- Task #155: `.env.example` with placeholders ✅

**Enables:**
- Task #157: Docker Compose secrets integration
- Task #158: Production deployment guide
