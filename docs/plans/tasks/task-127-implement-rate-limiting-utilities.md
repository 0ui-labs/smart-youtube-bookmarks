# Task #127: Implement Rate Limiting Utilities

**Plan Task:** #127
**Wave/Phase:** Security Hardening - Task 4: API Rate Limiting (Steps 2-3)
**Dependencies:** Task #126 (slowapi dependency installed), Task #120 (Environment-aware configuration with `env` field)
...
def get_rate_limit_key(request: Request) -> str:
    ...
    Implementation Notes:
        - Checks request.state.user (set by auth dependency after Task #113)
...
# In future endpoint files (Task #113, etc.)
from app.core.rate_limit import limiter, AUTH_RATE_LIMIT
...
**NOT in scope:**
- Applying rate limits to specific endpoints (future tasks: Task #113, etc.)
...
**Requires:**
- **Task #126:** slowapi dependency installed (`slowapi==0.1.9`)
...
1. **Task #113: Protect API endpoints with JWT authentication**
...
- [ ] Ready for Task #113 (apply to endpoints)
