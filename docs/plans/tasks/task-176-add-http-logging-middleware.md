# Task #176: Structured Logging - Add HTTP Request Logging Middleware

**Plan Task:** #176
**Wave/Phase:** P2 Operational Excellence - Wave 3
**Dependencies:** Task #175 (logging.py module must exist)
...
- [ ] Uses `log_request()` helper from Task #175's `app.core.logging` module
...
**Options:**
1. `request.state.user` → Set by auth dependency (Task #150)
...
- Auth dependency (Task #150) will set `request.state.user` on authenticated requests
...
- ⚠️ Requires Task #150 to be completed for user_id logging (gracefully degrades to None)
...
async def test_logging_middleware_no_user_id(client: AsyncClient, log_stream: StringIO):
    ...
    # After Task #150, authenticated requests will include user_id
    assert "http_request" in log_output
...
async def test_logging_middleware_with_user_id(client: AsyncClient, log_stream: StringIO):
    ...
    # Note: This test will be skipped/xfail until Task #150 (auth dependency) is implemented
    # After Task #150, mock request.state.user to test user_id logging
    
    pytest.skip("Requires Task #150 (get_current_user dependency) to be implemented")
    
    # Future implementation after Task #150:
...
2. **Testing**
   ...
   - [ ] User ID test passes (or skipped with note for Task #150)
...
**Depends On:**
- Task #175: Create logging.py module with configure_logging(), get_logger(), log_request()

**Blocks:**
- Task #177: Migrate videos.py to structured logging (uses get_logger())
- Task #178: Update all API routers to use structured logging

**Related:**
- Task #150: Get current user dependency (will populate request.state.user)
- Task #158: Environment enum settings (settings.is_development for log format)
...
- User ID logging will be None until Task #150 (auth dependency) is complete
...
- Consider adding request ID to logs in future (Task #179: Request ID middleware)
