...
**Dependencies:** Task #158 (Environment Enum Settings), Task #159 (CORS Helper Functions)
...
**Rationale:** Import `settings` object for `debug` flag and environment value, plus the three CORS helper functions defined in Task #159.
...
- **Config helper functions:** `backend/app/core/config.py` (Task #159 - `get_cors_origins()`, `get_cors_methods()`, `get_cors_headers()`)
- **Environment enum:** `backend/app/core/config.py` (Task #158 - `Environment` enum)
...
- **Production validation:** Config validates that `ALLOWED_ORIGINS` is set in production (Task #159)
