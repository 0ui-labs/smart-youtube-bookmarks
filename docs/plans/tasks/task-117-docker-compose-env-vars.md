# Task #117: Secure Default Credentials - Update docker-compose.yml

**Plan Task:** #117
**Wave/Phase:** Phase 1: Security Hardening (Production Readiness)
**Dependencies:** Task #116 (Secret generation script - creates .env.example with REDIS_PASSWORD)
...
- [ ] Manual test: Services start successfully WITH .env from Task #116
...
**Expected Behavior:**
- ✅ WITH .env: Service starts with strong password from Task #116
...
- **Environment Variable:** `${REDIS_PASSWORD}` comes from .env file (Task #116 adds this)
...
POSTGRES_USER=user
POSTGRES_PASSWORD=changeme  # ⚠️ REPLACE with Task #116 generated secret

# Redis
REDIS_PASSWORD=changeme  # ⚠️ NEW from Task #116
...
**Verification:**
- ✅ Task #116 should have added `REDIS_PASSWORD` to .env.example
- ✅ Updated `REDIS_URL` to include password in connection string
- ⚠️ If Task #116 didn't add these, add them now
...
```bash
# Ensure .env exists with Task #116 generated secrets
...
```bash
# Generate secrets using Task #116 script
...
- REDIS_PASSWORD (required, new from Task #116)
- REDIS_URL format: redis://:password@redis:6379

Testing Verified:
- ✅ docker-compose up fails gracefully without .env
- ✅ Services start successfully with .env from Task #116
...
Depends on Task #116 (Secret generation script)
...
**Steps:**
1. Ensure `.env` exists with Task #116 generated secrets
2. Run `docker-compose up -d`
...
4. Verify strong passwords from Task #116 are used
...
- `.env.example` - Example environment variables (Task #116 adds REDIS_PASSWORD)
...
- ✅ Task #116 should have updated .env.example with new format
...
### Risk 3: Task #116 Incomplete (.env.example Missing REDIS_PASSWORD)

**Risk:** If Task #116 didn't add REDIS_PASSWORD, this task will fail
...
**Fallback Action:**
```bash
# If Task #116 incomplete, add to .env.example
...
# First-time setup
cp .env.example .env
python scripts/generate_secrets.py  # Task #116 script
