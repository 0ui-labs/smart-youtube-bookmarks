# Task #100: Create FieldsList Component for Global Field Overview
...
- Provide foundation for Task #101 (edit/delete actions)
...
export const FieldsList: React.FC<FieldsListProps> = ({
  ...
  const columns = useMemo(
    () => [
      ...
      // Actions Column (placeholder for Task #101)
...
**Current Implementation:** Frontend receives `CustomField[]` without usage_count (Task #100)

**Rationale for Future (Task #101):**
...
**Decision for Task #100:** Component supports `usage_count` prop structure, but backend doesn't provide it yet. Will display "0 schemas" placeholder until Task #101 extends backend endpoint.

**Migration Path:**
```typescript
// Step 1 (Task #100): Frontend accepts usage_count prop
interface CustomFieldRow extends CustomField {
  usage_count?: number; // Optional for now
}

// Step 2 (Task #101): Backend adds usage_count to response
...
- No backend modifications required for Task #100
...
1. **Usage Count Placeholder:** Component structure supports usage_count, but backend doesn't provide it yet (Task #101 will add this)
...
**Next Steps After Task #100:**
- Task #101: Add field actions (edit, delete, show usage count) - extends this component
- Task #102: Implement schema templates (predefined common schemas)
