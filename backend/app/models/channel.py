...
class Channel(BaseModel):
    ...
    name: Mapped[str] = mapped_column(
        String(255), nullable=False, comment="Channel display name from YouTube"
    )
    thumbnail_url: Mapped[str | None] = mapped_column(
        String(500), nullable=True, comment="Channel avatar URL from YouTube"
    )
    description: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="Channel description from YouTube"
    )
    ...
    videos: Mapped[list["Video"]] = relationship(
        "Video", back_populates="channel_ref", lazy="raise"
    )

    __table_args__ = (
        Index("idx_channels_user_id", "user_id"),
        Index(
            "idx_channels_user_youtube", "user_id", "youtube_channel_id", unique=True
        ),
    )
