...
async def publish_progress(
    ...
    try:
        ...
        logger.warning(
            f"✓ Published progress: video={video_id[:8]}... {progress}% ({stage}) -> channel={channel}"
        )
...
async def enrich_video(ctx: dict, video_id: str) -> dict:
    ...
    logger.warning(
        f"[enrich_video] Starting enrichment for {video_id}, redis available: {redis is not None}"
    )
                ...
                if redis:
                    list_result = await db.execute(
                        select(BookmarkList.user_id).where(
                            BookmarkList.id == video.list_id
                        )
                    )
                    user_id = list_result.scalar_one_or_none()
                    if user_id:
                        await publish_progress(
                            redis, str(user_id), video_id, 100, "error"
                        )
