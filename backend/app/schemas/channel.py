...
class ChannelBase(BaseModel):
    ...
    youtube_channel_id: str = Field(
        ..., min_length=1, max_length=50, description="YouTube channel ID"
    )
    thumbnail_url: str | None = Field(
        None, max_length=500, description="Channel avatar URL"
    )
    description: str | None = Field(
        None, description="Channel description from YouTube"
    )
    is_hidden: bool = Field(
        False, description="Hidden channels don't appear in sidebar"
    )
