...
router = APIRouter(
    prefix="/api/lists/{list_id}/schemas/{schema_id}/fields", tags=["schema-fields"]
)


async def validate_max_show_on_card(
    db: AsyncSession, schema_id: UUID, exclude_field_id: UUID | None = None
) -> None:
    """
    ...
    stmt = (
        ...
        .where(SchemaField.schema_id == schema_id, SchemaField.show_on_card == True)
    )
...
@router.post(
    "", response_model=SchemaFieldResponse, status_code=status.HTTP_201_CREATED
)
async def add_field_to_schema(
    ...
    field_stmt = select(CustomField).where(
        CustomField.id == schema_field.field_id, CustomField.list_id == list_id
    )
...
async def batch_update_schema_fields(
    ...
    existing_show_on_card = {
        sf.field_id: sf.show_on_card for sf in existing_schema_fields
    }
    ...
    return SchemaFieldBatchUpdateResponse(
        updated_count=len(request.fields), fields=updated_fields
    )
...
async def update_schema_field(
    ...
    stmt = select(SchemaField).where(
        SchemaField.schema_id == schema_id, SchemaField.field_id == field_id
    )
    result = await db.execute(stmt)
    ...
    stmt = (
        select(SchemaField)
        .where(SchemaField.schema_id == schema_id, SchemaField.field_id == field_id)
        .options(selectinload(SchemaField.field))
    )
    result = await db.execute(stmt)
    return result.scalar_one()
...
async def remove_field_from_schema(
    list_id: UUID, schema_id: UUID, field_id: UUID, db: AsyncSession = Depends(get_db)
):
    """
