...
async def test_create_custom_field_list_not_found(client: AsyncClient):
    ...
    field_data = {"name": "Test Field", "field_type": "boolean", "config": {}}

    response = await client.post(
        f"/api/lists/{fake_list_id}/custom-fields", json=field_data
    )
...
@pytest.mark.asyncio
async def test_create_custom_field_invalid_config(
    client: AsyncClient, test_list: BookmarkList
):
...
async def test_update_custom_field_name(
    client: AsyncClient, test_list: BookmarkList, rating_field: CustomField
):
    """Test updating field name."""
    update_data = {"name": "Updated Rating"}

    response = await client.put(
        f"/api/lists/{test_list.id}/custom-fields/{rating_field.id}", json=update_data
    )
...
async def test_update_custom_field_not_found(
    ...
    response = await client.put(
        f"/api/lists/{test_list.id}/custom-fields/{fake_field_id}", json=update_data
    )
...
@pytest.mark.asyncio
async def test_delete_custom_field_not_found(
    client: AsyncClient, test_list: BookmarkList
):
...
@pytest.mark.asyncio
async def test_duplicate_check_not_exists(client: AsyncClient, test_list: BookmarkList):
...
async def test_duplicate_check_scoped_to_list(
    ...
    field = CustomField(
        list_id=test_list.id, name="Field A", field_type="text", config={}
    )
    test_db.add(field)

    # Create another list with same field name
    other_list_id = uuid.uuid4()
    other_list = BookmarkList(id=other_list_id, name="Other List", user_id=test_user.id)
    test_db.add(other_list)

    other_field = CustomField(
        list_id=other_list_id, name="Field A", field_type="text", config={}
    )
    ...
    assert response.status_code == 200
    assert response.json()["exists"] is True
    assert response.json()["field"]["id"] == str(
        other_field.id
    )  # Returns other_list field


@pytest.mark.asyncio
async def test_duplicate_check_invalid_list_id(client: AsyncClient):
...
@pytest.mark.asyncio
async def test_duplicate_check_empty_name(client: AsyncClient, test_list: BookmarkList):
    """Test that empty name is rejected by Pydantic schema validation."""
    response = await client.post(
        f"/api/lists/{test_list.id}/custom-fields/check-duplicate", json={"name": ""}
    )
