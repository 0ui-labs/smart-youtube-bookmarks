...
async def test_get_analytics_success(client: AsyncClient, test_db: AsyncSession):
    ...
    # Create video
    video = Video(
        id=uuid4(), list_id=test_list.id, youtube_id="abc12345678", title="Test Video"
    )
    test_db.add(video)

    # Create field value
    field_value = VideoFieldValue(
        id=uuid4(), video_id=video.id, field_id=field.id, value_numeric=5
    )
...
async def test_most_used_fields_calculation(client: AsyncClient, test_db: AsyncSession):
    ...
    # Create 3 fields
    field1 = CustomField(
        id=uuid4(), list_id=test_list.id, name="Field 1", field_type="rating", config={}
    )
    field2 = CustomField(
        id=uuid4(), list_id=test_list.id, name="Field 2", field_type="rating", config={}
    )
    field3 = CustomField(
        id=uuid4(), list_id=test_list.id, name="Field 3", field_type="rating", config={}
    )
    ...
    videos = [
        Video(
            id=uuid4(), list_id=test_list.id, youtube_id=f"vid{i}", title=f"Video {i}"
        )
        for i in range(5)
    ]
    ...
    for video in videos:
        test_db.add(
            VideoFieldValue(
                id=uuid4(), video_id=video.id, field_id=field1.id, value_numeric=5
            )
        )
    ...
    for i in range(3):
        test_db.add(
            VideoFieldValue(
                id=uuid4(), video_id=videos[i].id, field_id=field2.id, value_numeric=3
            )
        )

    test_db.add(
        VideoFieldValue(
            id=uuid4(), video_id=videos[0].id, field_id=field3.id, value_numeric=1
        )
    )
...
async def test_unused_schemas_no_values(client: AsyncClient, test_db: AsyncSession):
    ...
    # Create schema
    schema = FieldSchema(
        id=uuid4(), list_id=test_list.id, name="Schema With Tag", description=None
    )
    test_db.add(schema)
...
@pytest.mark.asyncio
async def test_field_coverage_sorted_ascending(
    ...
    # Create 2 fields
    low_coverage_field = CustomField(
        id=uuid4(),
        list_id=test_list.id,
        name="Low Coverage",
        field_type="text",
    ...
    high_coverage_field = CustomField(
        id=uuid4(),
        list_id=test_list.id,
        name="High Coverage",
        field_type="text",
    ...
    # Create 10 videos
    videos = [
        Video(id=uuid4(), list_id=test_list.id, youtube_id=f"v{i}", title=f"Video {i}")
        for i in range(10)
    ]
    test_db.add_all(videos)
    ...
    test_db.add(
        VideoFieldValue(
            ...
            field_id=low_coverage_field.id,
            value_text="test",
        )
    )
    ...
    for i in range(9):
        test_db.add(
            VideoFieldValue(
                ...
                field_id=high_coverage_field.id,
                value_text="test",
            )
        )
...
@pytest.mark.asyncio
async def test_schema_effectiveness_calculation(
    client: AsyncClient, test_db: AsyncSession
):
    ...
    schema = FieldSchema(
        id=uuid4(), list_id=test_list.id, name="Test Schema", description=None
    )
    test_db.add(schema)
    ...
    for field in fields:
        test_db.add(
            VideoFieldValue(
                id=uuid4(), video_id=video1.id, field_id=field.id, value_numeric=5
            )
        )
    ...
    for i in range(2):
        test_db.add(
            VideoFieldValue(
                id=uuid4(), video_id=video2.id, field_id=fields[i].id, value_numeric=4
            )
        )
