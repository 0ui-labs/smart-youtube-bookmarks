...
async def test_fields(
    test_db: AsyncSession, test_list: BookmarkList
) -> list[CustomField]:
...
async def test_get_schema_fields_empty(
    client: AsyncClient, test_list: BookmarkList, test_schema: FieldSchema
):
...
async def test_post_duplicate_field_conflict(
    ...
    schema_field = SchemaField(
        schema_id=test_schema.id, field_id=test_field.id, display_order=0
    )
...
async def test_post_max_show_on_card_violation(
    ...
    for idx, fid in enumerate([field1.id, field2.id, field3.id]):
        schema_field = SchemaField(
            schema_id=test_schema.id, field_id=fid, display_order=idx, show_on_card=True
        )
        test_db.add(schema_field)
    await test_db.commit()
...
async def test_field_from_different_list_rejected(
    ...
    other_list = BookmarkList(
        user_id=test_user.id, name="Other List", description="Another list"
    )
    ...
    other_field = CustomField(
        list_id=other_list.id, name="Other Field", field_type="text", config={}
    )
...
async def test_put_toggle_show_on_card_validates_max(
    ...
    for idx, (fid, show) in enumerate(
        [(field1.id, True), (field2.id, True), (field3.id, True), (field4.id, False)]
    ):
        schema_field = SchemaField(
            schema_id=test_schema.id, field_id=fid, display_order=idx, show_on_card=show
        )
        test_db.add(schema_field)
    await test_db.commit()
...
async def test_delete_schema_not_found(
    client: AsyncClient, test_list: BookmarkList, test_field: CustomField
):
