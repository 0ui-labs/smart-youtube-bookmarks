...
async def test_start_processing_no_pending_videos(client):
    ...
    list_response = await client.post("/api/lists", json={"name": "Empty List"})
    list_id = list_response.json()["id"]
...
async def test_get_progress_history(client, test_db, test_user, test_list):
    ...
    job = ProcessingJob(list_id=test_list.id, total_videos=10, status="running")
    test_db.add(job)
    await test_db.flush()
    ...
    response = await client.get(
        f"/api/jobs/{job.id}/progress-history", params={"user_id": str(test_user.id)}
    )
...
@pytest.mark.asyncio
async def test_get_progress_history_with_since_filter(
    client, test_db, test_user, test_list
):
    ...
    response = await client.get(
        ...
        params={"user_id": str(test_user.id), "since": since_time.isoformat()},
    )
...
async def test_get_progress_history_pagination(client, test_db, test_user, test_list):
    ...
    response = await client.get(
        f"/api/jobs/{job.id}/progress-history",
        params={"user_id": str(test_user.id), "offset": 5, "limit": 3},
    )
...
async def test_get_progress_history_unauthorized(client, test_db, test_user, test_list):
    ...
    other_list = BookmarkList(name="Other User's List", user_id=other_user.id)
    test_db.add(other_list)
    await test_db.flush()
    await test_db.refresh(other_list)

    # Create job for other user
    other_job = ProcessingJob(list_id=other_list.id, total_videos=5, status="running")
