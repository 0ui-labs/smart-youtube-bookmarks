...
@pytest.mark.asyncio
async def test_add_video_short_url(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList
):
...
@pytest.mark.asyncio
async def test_add_video_embed_url(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList
):
...
@pytest.mark.asyncio
async def test_add_video_mobile_url(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList
):
...
@pytest.mark.asyncio
async def test_add_video_with_query_params(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList
):
...
@pytest.mark.asyncio
async def test_add_duplicate_video(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList
):
...
@pytest.mark.asyncio
async def test_add_video_to_nonexistent_list(
...
@pytest.mark.asyncio
async def test_add_video_invalid_url_format(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList
):
...
@pytest.mark.asyncio
async def test_add_video_non_youtube_domain(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList
):
    """Test adding a video from non-YouTube domain is rejected."""
    response = await client.post(
        f"/api/lists/{test_list.id}/videos", json={"url": "https://vimeo.com/12345678"}
    )
...
@pytest.mark.asyncio
async def test_add_video_http_protocol(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList
):
...
@pytest.mark.asyncio
async def test_add_video_unicode_chars(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList
):
...
@pytest.mark.asyncio
async def test_get_videos_in_list(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList
):
...
@pytest.mark.asyncio
async def test_get_videos_empty_list(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList
):
...
async def test_bulk_upload_csv_success(client, test_list, monkeypatch):
    ...
    mock_youtube_client.get_batch_metadata = AsyncMock(
        return_value=[
            {
...
@pytest.mark.asyncio
async def test_bulk_upload_fetches_youtube_metadata(
    client, test_list, test_db, monkeypatch
):
    ...
    with (
        patch("app.api.videos.YouTubeClient") as mock_youtube_client_class,
...
@pytest.mark.asyncio
async def test_get_videos_includes_tags(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList
):
    ...
    tag1_response = await client.post(
        "/api/tags", json={"name": "Python-test", "color": "#3B82F6"}
    )
    tag2_response = await client.post(
        "/api/tags", json={"name": "Tutorial-test", "color": "#10B981"}
    )
...
@pytest.mark.asyncio
async def test_get_videos_filter_by_single_tag(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList
):
    ...
    python_tag = await client.post(
        "/api/tags", json={"name": "SingleTag-Python", "color": "#3B82F6"}
    )
    js_tag = await client.post(
        "/api/tags", json={"name": "SingleTag-JavaScript", "color": "#F59E0B"}
    )
    ...
    await client.post(
        f"/api/videos/{video3_id}/tags", json={"tag_ids": [python_id, js_id]}
    )

    # Filter by Python tag
    response = await client.get(
        f"/api/lists/{test_list.id}/videos?tags=SingleTag-Python"
    )
...
@pytest.mark.asyncio
async def test_get_videos_filter_by_multiple_tags_or_logic(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList
):
    ...
    # Create tags with unique names for this test
    python_tag = await client.post(
        "/api/tags", json={"name": "MultiTag-Python", "color": "#3B82F6"}
    )
    tutorial_tag = await client.post(
        "/api/tags", json={"name": "MultiTag-Tutorial", "color": "#10B981"}
    )
    advanced_tag = await client.post(
        "/api/tags", json={"name": "MultiTag-Advanced", "color": "#EF4444"}
    )
    python_id = python_tag.json()["id"]
    ...
    await client.post(
        f"/api/videos/{video3_id}/tags", json={"tag_ids": [python_id, tutorial_id]}
    )
    ...
    response = await client.get(
        f"/api/lists/{test_list.id}/videos?tags=MultiTag-Python&tags=MultiTag-Tutorial"
    )
...
@pytest.mark.asyncio
async def test_get_videos_filter_case_insensitive(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList
):
    ...
    tag = await client.post(
        "/api/tags", json={"name": "CaseTest-PyThOn", "color": "#3B82F6"}
    )
    tag_id = tag.json()["id"]
    ...
    response1 = await client.get(
        f"/api/lists/{test_list.id}/videos?tags=casetest-python"
    )
...
@pytest.mark.asyncio
async def test_get_videos_filter_no_matching_tags(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList
):
    ...
    tag = await client.post(
        "/api/tags", json={"name": "NoMatch-Python", "color": "#3B82F6"}
    )
    tag_id = tag.json()["id"]
    await client.post(f"/api/videos/{video_id}/tags", json={"tag_ids": [tag_id]})
    ...
    response = await client.get(
        f"/api/lists/{test_list.id}/videos?tags=NonExistent-XYZ"
    )
...
@pytest.mark.asyncio
async def test_get_videos_without_filter_returns_all(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList
):
    ...
    tag = await client.post(
        "/api/tags", json={"name": "AllVideos-Python", "color": "#3B82F6"}
    )
    await client.post(
        f"/api/videos/{video1_id}/tags", json={"tag_ids": [tag.json()["id"]]}
    )
...
@pytest.mark.asyncio
async def test_get_videos_field_values_empty_when_no_tags(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList
):
...
@pytest.mark.asyncio
async def test_get_videos_field_values_union_from_multiple_schemas(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList
):
    ...
    tag1_response = await client.post(
        "/api/tags", json={"name": "Tag1", "color": "#3B82F6"}
    )
    tag1_id = tag1_response.json()["id"]
    ...
    tag2_response = await client.post(
        "/api/tags", json={"name": "Tag2", "color": "#10B981"}
    )
    tag2_id = tag2_response.json()["id"]
...
@pytest.mark.asyncio
async def test_get_videos_field_values_rating_accepts_float(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList, test_user: User
):
    ...
    tag_response = await client.post(
        f"/api/tags?user_id={test_user.id}", json={"name": "Rated", "color": "#F59E0B"}
    )
    tag_id = tag_response.json()["id"]
    await client.put(
        f"/api/tags/{tag_id}?user_id={test_user.id}", json={"schema_id": schema_id}
    )
    ...
    value = VideoFieldValue(
        video_id=UUID(video_id), field_id=UUID(field_id), value_numeric=4.5
    )
    test_db.add(value)
...
async def test_sort_by_title_ascending(
    ...
    # Mock YouTube client
    mock_youtube_client = AsyncMock()
    mock_youtube_client.get_video_metadata = AsyncMock(
        return_value={
            "youtube_id": "test123",
            "title": "Test Video",
    ...
    response = await client.get(
        f"/api/lists/{test_list.id}/videos?sort_by=title&sort_order=asc"
    )
...
async def test_sort_by_title_descending(
    ...
    response = await client.get(
        f"/api/lists/{test_list.id}/videos?sort_by=title&sort_order=desc"
    )
...
@pytest.mark.asyncio
async def test_sort_by_duration_ascending(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList
):
    ...
    await test_db.execute(
        update(Video)
        .where(Video.youtube_id == "LongVideo01")
        .values(title="Long Video", duration=600)
    )
    await test_db.execute(
        update(Video)
        .where(Video.youtube_id == "ShortVideo1")
        .values(title="Short Video", duration=120)
    )
    await test_db.execute(
        update(Video)
        .where(Video.youtube_id == "MediumVideo")
        .values(title="Medium Video", duration=300)
    )
    await test_db.commit()

    # Test sorting by duration ascending
    response = await client.get(
        f"/api/lists/{test_list.id}/videos?sort_by=duration&sort_order=asc"
    )
...
@pytest.mark.asyncio
async def test_sort_by_duration_descending(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList
):
    ...
    response = await client.get(
        f"/api/lists/{test_list.id}/videos?sort_by=duration&sort_order=desc"
    )
...
@pytest.mark.asyncio
async def test_sort_by_field_rating_descending(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList
):
    ...
    field_response = await client.post(
        f"/api/lists/{test_list.id}/custom-fields",
        json={"name": "Quality", "field_type": "rating", "config": {"max_rating": 5}},
    )
    field_id = field_response.json()["id"]
    ...
    value_high = VideoFieldValue(
        video_id=UUID(video_high_id), field_id=UUID(field_id), value_numeric=5
    )
    value_low = VideoFieldValue(
        video_id=UUID(video_low_id), field_id=UUID(field_id), value_numeric=2
    )
    ...
    await test_db.execute(
        update(Video)
        .where(Video.id == UUID(video_high_id))
        .values(title="High Quality")
    )
    await test_db.execute(
        update(Video).where(Video.id == UUID(video_low_id)).values(title="Low Quality")
    )
    await test_db.execute(
        update(Video)
        .where(Video.id == UUID(video_no_rating_id))
        .values(title="No Rating")
    )
    await test_db.commit()
...
@pytest.mark.asyncio
async def test_sort_by_field_rating_ascending_nulls_last(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList
):
    ...
    field_response = await client.post(
        f"/api/lists/{test_list.id}/custom-fields",
        json={"name": "Rating", "field_type": "rating", "config": {"max_rating": 5}},
    )
    field_id = field_response.json()["id"]
    ...
    value_v1 = VideoFieldValue(
        video_id=UUID(v1_id), field_id=UUID(field_id), value_numeric=3
    )
    value_v3 = VideoFieldValue(
        video_id=UUID(v3_id), field_id=UUID(field_id), value_numeric=1
    )
    ...
    # Update titles using UPDATE statement
    await test_db.execute(
        update(Video).where(Video.id == UUID(v1_id)).values(title="Video 1")
    )
    await test_db.execute(
        update(Video).where(Video.id == UUID(v2_id)).values(title="Video 2")
    )
    await test_db.execute(
        update(Video).where(Video.id == UUID(v3_id)).values(title="Video 3")
    )
    await test_db.commit()
...
@pytest.mark.asyncio
async def test_sort_by_field_select_ascending(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList
):
    ...
    # Set field values
    value_high = VideoFieldValue(
        video_id=UUID(video_high_id), field_id=UUID(field_id), value_text="High"
    )
    value_low = VideoFieldValue(
        video_id=UUID(video_low_id), field_id=UUID(field_id), value_text="Low"
    )
    value_medium = VideoFieldValue(
        video_id=UUID(video_medium_id), field_id=UUID(field_id), value_text="Medium"
    )
    test_db.add(value_high)
    test_db.add(value_low)
    ...
    await test_db.execute(
        update(Video)
        .where(Video.id == UUID(video_high_id))
        .values(title="High Priority")
    )
    await test_db.execute(
        update(Video).where(Video.id == UUID(video_low_id)).values(title="Low Priority")
    )
    await test_db.execute(
        update(Video)
        .where(Video.id == UUID(video_medium_id))
        .values(title="Medium Priority")
    )
    await test_db.commit()
...
@pytest.mark.asyncio
async def test_sort_by_invalid_field_id(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList
):
...
@pytest.mark.asyncio
async def test_sort_by_invalid_column(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList
):
...
@pytest.mark.skip(
    reason="PostgreSQL DISTINCT + ORDER BY limitation with joined tables - requires subquery fix"
)
@pytest.mark.asyncio
async def test_sort_combined_with_tag_filter(
    client: AsyncClient, test_db: AsyncSession, test_list: BookmarkList
):
    ...
    # Set field values
    value_v1 = VideoFieldValue(
        video_id=UUID(v1_id), field_id=UUID(field_id), value_numeric=2
    )
    value_v2 = VideoFieldValue(
        video_id=UUID(v2_id), field_id=UUID(field_id), value_numeric=5
    )
    value_v3 = VideoFieldValue(
        video_id=UUID(v3_id), field_id=UUID(field_id), value_numeric=4
    )
    test_db.add(value_v1)
    ...
    # Update titles using UPDATE statement
    await test_db.execute(
        update(Video).where(Video.id == UUID(v1_id)).values(title="Low Rated")
    )
    await test_db.execute(
        update(Video).where(Video.id == UUID(v2_id)).values(title="High Rated")
    )
    await test_db.execute(
        update(Video).where(Video.id == UUID(v3_id)).values(title="No Tag")
    )
    await test_db.commit()
...
async def test_default_sort_created_at_desc(
    ...
    await test_db.execute(
        update(Video).where(Video.id == UUID(old_id)).values(title="Old Video")
    )
    await test_db.execute(
        update(Video).where(Video.id == UUID(new_id)).values(title="New Video")
    )
    await test_db.commit()
...
async def test_get_video_detail_with_available_fields_but_no_values(
    ...
    # Assign tag to video
    tag_response = await client.post(
        f"/api/videos/{video_id}/tags", json={"tag_ids": [str(tag.id)]}
    )
...
async def test_get_video_detail_with_multiple_field_values(
    ...
    boolean_field = CustomField(
        list_id=test_list.id, name="Watched", field_type="boolean", config={}
    )
    ...
    for i, field in enumerate([rating_field, select_field, boolean_field], start=1):
        schema_field = SchemaField(
            schema_id=schema.id, field_id=field.id, display_order=i, show_on_card=True
        )
        test_db.add(schema_field)
    await test_db.commit()

    # Create a tag linked to the schema
    tag = Tag(name="Test Tag With Schema", user_id=test_user.id, schema_id=schema.id)
    test_db.add(tag)
    await test_db.commit()
    await test_db.refresh(tag)
