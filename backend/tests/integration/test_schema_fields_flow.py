...
async def test_schema_fields_full_workflow(
    ...
    schema_response = await client.post(
        ...
        json={"name": "Video Quality", "description": "Quality metrics"},
    )
    ...
    for idx, field_id in enumerate(field_ids[:3]):
        add_response = await client.post(
            ...
            json={"field_id": field_id, "show_on_card": True},
        )
    ...
    fail_response = await client.post(
        f"/api/lists/{test_list.id}/schemas/{schema_id}/fields",
        json={"field_id": field_ids[3], "show_on_card": True},
    )
    ...
    success_response = await client.post(
        f"/api/lists/{test_list.id}/schemas/{schema_id}/fields",
        json={"field_id": field_ids[3], "show_on_card": False},
    )
    ...
    all_fields_response = await client.get(f"/api/lists/{test_list.id}/custom-fields")
