...
async def test_cascade_delete_field_removes_values(
    ...
    video1 = Video(list_id=test_list.id, youtube_id="video_001", processing_status="completed")
    video2 = Video(list_id=test_list.id, youtube_id="video_002", processing_status="completed")
    ...
    assert len(remaining_values) == 0, "VideoFieldValue records should be CASCADE deleted by database"
...
async def test_cascade_delete_schema_removes_join_entries(
    ...
    assert len(remaining_joins) == 0, "SchemaField join entries should be CASCADE deleted by database"
    ...
    assert len(remaining_fields) == 2, "CustomField records should NOT be deleted (fields are reusable)"
...
async def test_cascade_delete_schema_sets_tag_null(
    ...
    assert remaining_tag is not None, "Tag should NOT be deleted (ON DELETE SET NULL, not CASCADE)"
    assert remaining_tag.schema_id is None, "Tag.schema_id should be SET NULL by database CASCADE"
