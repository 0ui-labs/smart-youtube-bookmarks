...
async def test_schema_full_lifecycle(
    client: AsyncClient, test_db, test_list: BookmarkList
):
    """Test complete schema lifecycle: create → update → delete."""
    # Create schema
    create_data = {"name": "Video Quality", "description": "Quality metrics"}
    create_response = await client.post(
        f"/api/lists/{test_list.id}/schemas", json=create_data
    )
    ...
    update_response = await client.put(
        f"/api/lists/{test_list.id}/schemas/{schema_id}", json=update_data
    )
...
async def test_schema_with_fields_eager_loading(
    ...
    create_data = {
        ...
        "fields": [
            ...
            {"field_id": str(field2.id), "display_order": 1, "show_on_card": True},
        ],
    }
    create_response = await client.post(
        f"/api/lists/{test_list.id}/schemas", json=create_data
...
async def test_cannot_delete_schema_bound_to_tag(
    client: AsyncClient, test_db, test_list: BookmarkList, test_user: User
):
