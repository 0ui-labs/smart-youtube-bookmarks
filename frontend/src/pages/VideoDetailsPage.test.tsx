...
import type { VideoResponse, VideoFieldValue, AvailableFieldResponse } from '@/types/video'
...
// Mock useTags hooks (for CategorySelector)
// Categories include Tutorial (from mockVideo.tags) so CategorySelector can display it
vi.mock('@/hooks/useTags', () => ({
  useTags: vi.fn(() => ({
    data: [
      { id: 'cat-1', name: 'Tutorial', color: '#3B82F6', is_video_type: true, schema_id: null, user_id: 'user-1', created_at: '2024-01-01T00:00:00Z', updated_at: '2024-01-01T00:00:00Z' },
    ],
    isLoading: false,
    error: null,
  })),
...
// Mock useSetVideoCategory hook (for CategorySelector)
...
const createAvailableField = (overrides?: Partial<AvailableFieldResponse>): AvailableFieldResponse => ({
  field_id: 'field-1',
  field_name: 'Rating',
  field_type: 'rating',
...
const mockVideo: VideoResponse = {
  ...
  tags: [
    { id: 'cat-1', name: 'Tutorial', color: '#3B82F6', is_video_type: true, schema_id: null, user_id: 'user-1', created_at: '2024-01-01T00:00:00Z', updated_at: '2024-01-01T00:00:00Z' },
    { id: 'label-1', name: 'Python', color: '#10B981', is_video_type: false, schema_id: null, user_id: 'user-1', created_at: '2024-01-01T00:00:00Z', updated_at: '2024-01-01T00:00:00Z' },
  ],
  ...
  available_fields: [
    createAvailableField({ field_id: 'field-1', field_name: 'Rating', field_type: 'rating', display_order: 0 }),
    createAvailableField({ field_id: 'field-2', field_name: 'Quality', field_type: 'select', display_order: 1, config: { options: ['Low', 'Medium', 'High'] } }),
    createAvailableField({ field_id: 'field-3', field_name: 'Notes', field_type: 'text', display_order: 2, config: { max_length: 500 }, show_on_card: false }),
  ],
}
...
describe('VideoDetailsPage', () => {
  ...
  describe('Loading and Error States', () => {
    ...
    it('renders error state on fetch failure', () => {
      ...
      expect(screen.getByText('Fehler beim Laden des Videos.')).toBeInTheDocument()
  ...
  describe('Video Header', () => {
    it('displays video title as h1', () => {
      ...
      const heading = screen.getByRole('heading', { level: 1, name: mockVideo.title! })
    ...
    it('displays category and labels separately', () => {
      renderWithRouter(<VideoDetailsPage />)

      // Category "Tutorial" should be shown in CategorySelector
      // Label "Python" should be shown as a badge
  ...
  describe('Field Grouping by Schema', () => {
    ...
    it('shows field count in section header', () => {
      ...
      // 3 available fields - now shows just the number in parentheses
      expect(screen.getByText(/tutorial schema \(3\)/i)).toBeInTheDocument()
    })

    it('shows count for single field', () => {
      ...
      // Now shows just the number in parentheses (no "Feld/Felder")
      expect(screen.getByText(/tutorial schema \(1\)/i)).toBeInTheDocument()
    })
    ...
    it('sorts schemas alphabetically', () => {
      const videoWithMultipleSchemas: VideoResponse = {
        ...mockVideo,
        available_fields: [
          createAvailableField({ field_id: 'field-1', schema_name: 'Zebra Schema', display_order: 0 }),
          createAvailableField({ field_id: 'field-2', schema_name: 'Alpha Schema', display_order: 0 }),
        ],
      }
      ...
      // Look for schema section headers - now shows just number in parentheses (not "Feld")
      expect(screen.getByText(/alpha schema \(1\)/i)).toBeInTheDocument()
      expect(screen.getByText(/zebra schema \(1\)/i)).toBeInTheDocument()

      // Alpha should appear before Zebra in DOM
      const sections = screen.getAllByRole('button').filter(btn =>
        btn.textContent?.includes('Schema')
      )
    ...
    it('shows "Keine Informationen verfügbar" when no fields', () => {
      ...
      // Updated terminology: "Informationen" instead of "benutzerdefinierten Felder"
      expect(screen.getByText('Keine Informationen verfügbar.')).toBeInTheDocument()
    })
    ...
    it('sorts fields by display_order within schema', () => {
      const videoWithUnsortedFields: VideoResponse = {
        ...mockVideo,
        available_fields: [
          createAvailableField({ field_id: 'field-3', field_name: 'Field C', display_order: 2 }),
          createAvailableField({ field_id: 'field-1', field_name: 'Field A', display_order: 0 }),
          createAvailableField({ field_id: 'field-2', field_name: 'Field B', display_order: 1 }),
        ],
      }
  ...
  describe('Field Rendering', () => {
    ...
    it('renders empty fields with null values', () => {
      const videoWithEmptyField: VideoResponse = {
        ...
        available_fields: [
          createAvailableField({ field_id: 'field-1', field_name: 'Empty Rating', field_type: 'rating', display_order: 0 }),
        ],
      }
  ...
  describe('Navigation', () => {
    it('back button navigates to /videos', async () => {
      ...
      const backButton = screen.getAllByRole('button', { name: /zurück zur übersicht/i })[0]
    ...
    it('channel button exists but does not navigate yet', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Channel clicked:', mockVideo.channel)
  ...
  describe('Integration: Full User Flows', () => {
    it('toggles schema section on click', async () => {
      ...
      const sectionButton = screen.getByRole('button', { name: /tutorial schema/i })
    ...
    it('handles null available_fields gracefully', () => {
      ...
      // Should show "no fields" message (updated terminology)
      expect(screen.getByText('Keine Informationen verfügbar.')).toBeInTheDocument()
    })
  })
  ...
  describe('Accessibility', () => {
    ...
    it('field sections have proper structure', () => {
      ...
      // Check that "Informationen" heading exists (renamed from "Benutzerdefinierte Felder")
      const fieldsHeading = screen.getByRole('heading', { level: 2, name: /informationen/i })
  ...
  describe('Edge Cases', () => {
    it('handles video without thumbnail', () => {
      ...
      expect(screen.getByRole('heading', { level: 1, name: mockVideo.title! })).toBeInTheDocument()
    })

    it('handles video without tags (shows empty category selector)', () => {
      ...
      // Should not crash, title still visible
      expect(screen.getByRole('heading', { level: 1, name: mockVideo.title! })).toBeInTheDocument()
      // Category selector should show "Keine Kategorie" or be present
    })
