...
export type ImportStage = 'created' | 'metadata' | 'captions' | 'chapters' | 'complete' | 'error';
  ...
  const startSyntheticProgress = (videoId: string) => {
    ...
    const timerId = window.setInterval(() => {
      ...
      if (current.targetProgress < ceiling) {
        const newTarget = Math.min(current.targetProgress + SYNTHETIC_INCREMENT, ceiling);
  ...
  return {
    ...
    clearProgress: (videoId) => {
      ...
      set((s) => {
        ...
        const newTimers = new Map(s.syntheticTimers);
        newTimers.delete(videoId);
        return { progress: newProgress, animationFrames: newFrames, syntheticTimers: newTimers };
      });
    },
    ...
    clearAllProgress: () => {
      ...
      set({ progress: new Map(), animationFrames: new Map(), syntheticTimers: new Map() });
    },
