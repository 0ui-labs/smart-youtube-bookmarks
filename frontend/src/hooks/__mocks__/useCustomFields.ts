...
export function useCheckFieldDuplicate(listId: string, name: string, options?: { enabled?: boolean }) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        exists,
        field: exists ? mockFields.find(f => f.name.toLowerCase() === name.toLowerCase()) : null,
      }
    },
