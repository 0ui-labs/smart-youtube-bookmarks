...
describe("useVideoFieldValues", () => {
  ...
  describe("useVideoFieldValues (query)", () => {
    it("should fetch field values successfully", async () => {
      const { result } = renderHook(() => useVideoFieldValues("video-123"), {
        wrapper,
      });
    ...
    it("should handle empty field values", async () => {
      const { result } = renderHook(() => useVideoFieldValues("video-empty"), {
        wrapper,
      });
    ...
    it("should handle API errors", async () => {
      const { result } = renderHook(() => useVideoFieldValues("video-404"), {
        wrapper,
      });
  ...
  describe("useUpdateVideoFieldValues (mutation)", () => {
    ...
    it("should handle validation errors", async () => {
      ...
      result.current.mutate([{ field_id: "field-invalid", value: 10 }]);
    ...
    it("should invalidate field values cache after successful mutation", async () => {
      ...
      result.current.mutate([{ field_id: "field-1", value: 5 }]);
    ...
    it("should update single field value", async () => {
      ...
      result.current.mutate([{ field_id: "field-1", value: 3 }]);
    ...
    it("should support clearing field value with null", async () => {
      ...
      result.current.mutate([{ field_id: "field-1", value: null }]);
