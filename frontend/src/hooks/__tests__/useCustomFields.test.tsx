...
describe("customFieldKeys", () => {
  it("generates correct hierarchical keys", () => {
    ...
    expect(customFieldKeys.list(mockListId)).toEqual([
      "custom-fields",
      "list",
      mockListId,
    ]);
    expect(customFieldKeys.details()).toEqual(["custom-fields", "detail"]);
    expect(customFieldKeys.detail("field-id")).toEqual([
      "custom-fields",
      "detail",
      "field-id",
    ]);
...
describe("useCustomFields", () => {
  ...
  it("fetches custom fields successfully", async () => {
    ...
    const { result } = renderHook(() => useCustomFields(mockListId), {
      wrapper,
    });
...
describe("useCreateCustomField", () => {
  ...
  it("creates custom field successfully", async () => {
    ...
    const { result } = renderHook(() => useCreateCustomField(mockListId), {
      wrapper,
    });
...
describe("useUpdateCustomField", () => {
  ...
  it("updates custom field successfully", async () => {
    ...
    const { result } = renderHook(() => useUpdateCustomField(mockListId), {
      wrapper,
    });
...
describe("useDeleteCustomField", () => {
  ...
  it("deletes custom field with optimistic update", async () => {
    ...
    const { result } = renderHook(() => useDeleteCustomField(mockListId), {
      wrapper,
    });
    ...
    expect(customFieldsApi.delete).toHaveBeenCalledWith(
      mockListId,
      mockField.id
    );
  });
