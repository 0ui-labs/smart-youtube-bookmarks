...
describe('useSchemas', () => {
  ...
  it('handles network errors gracefully', async () => {
    vi.mocked(schemasApi.getSchemas).mockRejectedValueOnce(new Error('Network error'))
...
describe('useSchema', () => {
  ...
  it('fetches single schema with nested fields', async () => {
    ...
    const { result } = renderHook(() => useSchema(mockListId, mockSchemaId), { wrapper })
  ...
  it('does not fetch when schemaId is undefined (dependent query)', async () => {
    const { result } = renderHook(() => useSchema(mockListId, undefined), { wrapper })
  ...
  it('returns 404 for non-existent schema', async () => {
    ...
    const { result } = renderHook(() => useSchema(mockListId, 'nonexistent'), { wrapper })
...
describe('useCreateSchema', () => {
  ...
  it('creates schema without fields', async () => {
    ...
    const { result } = renderHook(() => useCreateSchema(mockListId), { wrapper })
    ...
    expect(schemasApi.createSchema).toHaveBeenCalledWith(mockListId, schemaData)
  })
  ...
  it('invalidates schemas list after creation', async () => {
    ...
    // Fetch schemas first
    const { result: schemasResult } = renderHook(() => useSchemas(mockListId), { wrapper })
    ...
    // Create new schema
    const { result: createResult } = renderHook(() => useCreateSchema(mockListId), { wrapper })
    ...
    await waitFor(() => {
      expect(schemasResult.current.dataUpdatedAt).toBeGreaterThan(initialDataUpdateCount)
    })
  })
})
...
describe('useUpdateSchema', () => {
  ...
  it('updates schema name and description', async () => {
    ...
    const { result } = renderHook(() => useUpdateSchema(mockListId), { wrapper })
    ...
    expect(schemasApi.updateSchema).toHaveBeenCalledWith(mockListId, mockSchemaId, updates)
  })
  ...
  it('invalidates both list and detail queries', async () => {
    ...
    const { result } = renderHook(() => useUpdateSchema(mockListId), { wrapper })
    result.current.mutate({ schemaId: mockSchemaId, updates: { name: 'Updated' } })

    await waitFor(() => expect(result.current.isSuccess).toBe(true))

    // Check that both queries were invalidated
    const listState = queryClient.getQueryState(['schemas', 'list', mockListId])
    const detailState = queryClient.getQueryState(['schemas', 'detail', mockSchemaId])
...
describe('useDeleteSchema', () => {
  ...
  it('removes schema from cache', async () => {
    ...
    const cachedData = queryClient.getQueryData(['schemas', 'detail', mockSchemaId])
...
describe('useAddFieldToSchema', () => {
  ...
  it('adds field to schema', async () => {
    ...
    const { result } = renderHook(() => useAddFieldToSchema(mockListId, mockSchemaId), { wrapper })
    ...
    expect(schemasApi.addFieldToSchema).toHaveBeenCalledWith(mockListId, mockSchemaId, fieldData)
  })
...
describe('useRemoveFieldFromSchema', () => {
  ...
  it('removes field from schema', async () => {
    vi.mocked(schemasApi.removeFieldFromSchema).mockResolvedValueOnce(undefined)

    const { result } = renderHook(() => useRemoveFieldFromSchema(mockListId, mockSchemaId), { wrapper })

    result.current.mutate('field-1')

    await waitFor(() => expect(result.current.isSuccess).toBe(true))
    expect(schemasApi.removeFieldFromSchema).toHaveBeenCalledWith(mockListId, mockSchemaId, 'field-1')
  })
...
describe('useUpdateSchemaField', () => {
  ...
  it('updates field display_order', async () => {
    ...
    const { result } = renderHook(() => useUpdateSchemaField(mockListId, mockSchemaId), { wrapper })
    ...
    expect(schemasApi.updateSchemaField).toHaveBeenCalledWith(mockListId, mockSchemaId, 'field-1', updates)
  })
...
describe('useReorderSchemaFields', () => {
  ...
  it('applies optimistic update immediately', async () => {
    ...
    // Fetch schema first
    const { result: schemaResult } = renderHook(() => useSchema(mockListId, mockSchemaId), { wrapper })
    await waitFor(() => expect(schemaResult.current.isSuccess).toBe(true))

    // Reorder fields
    const { result: reorderResult } = renderHook(() => useReorderSchemaFields(mockListId, mockSchemaId), { wrapper })
    ...
    const optimisticData = queryClient.getQueryData(['schemas', 'detail', mockSchemaId])
  ...
  it('refetches after success to ensure consistency', async () => {
    ...
    const { result } = renderHook(() => useReorderSchemaFields(mockListId, mockSchemaId), { wrapper })
