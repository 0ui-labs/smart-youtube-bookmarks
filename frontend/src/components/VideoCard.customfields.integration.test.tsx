...
import type { VideoFieldValue, VideoResponse } from "@/types/video";
...
describe("VideoCard - Custom Fields Integration", () => {
  ...
  describe("Display Integration", () => {
    it("displays custom fields preview below tags", () => {
      ...
      expect(
        screen.getByLabelText(/Quality: 4 out of 5$/i)
      ).toBeInTheDocument();
    ...
    it("shows video with no custom fields without errors", () => {
      ...
      expect(
        screen.getByText("Tutorial: How to Build Custom Fields")
      ).toBeInTheDocument();
  ...
  describe("Inline Editing Integration", () => {
    it("allows inline editing of rating field with optimistic update", async () => {
      ...
      await waitFor(() => {
        expect(mockPut).toHaveBeenCalledWith("/videos/video-123/fields", {
    ...
    it("allows inline editing of select field", async () => {
      ...
      // Select 'great' option
      const greatOption = await screen.findByRole("menuitem", {
        name: "great",
      });
  ...
  describe("Event Isolation", () => {
    it("does not trigger video onClick when editing field", async () => {
      ...
      renderWithQuery(
        <VideoCard onClick={handleVideoClick} video={mockVideoWithFields} />
      );
  ...
  describe("More Fields Badge", () => {
    ...
    it("does not trigger video onClick when clicking more badge", async () => {
      ...
      renderWithQuery(
        <VideoCard onClick={handleVideoClick} video={mockVideoWithManyFields} />
      );
