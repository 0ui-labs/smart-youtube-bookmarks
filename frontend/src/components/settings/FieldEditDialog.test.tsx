...
describe('FieldEditDialog', () => {
  ...
  it('validates invalid JSON config', async () => {
    ...
    const configInput = screen.getByLabelText(/Configuration/i) as HTMLTextAreaElement
  ...
  it('resets form when dialog closes', async () => {
    ...
    const resetNameInput = screen.getByLabelText(/Field Name/i) as HTMLInputElement
  ...
  it('shows warning about config changes', () => {
    ...
    expect(screen.getByText(/Changing field configuration may affect existing values/i)).toBeInTheDocument()
  })
