...
describe('Robust Import Integration Flow', () => {
  ...
  describe('Phase 4: Import Progress Display', () => {
    ...
    it('shows progress overlay when video import starts', async () => {
      ...
      act(() => {
        useImportProgressStore.getState().setProgress('video-import-123', 25, 'metadata')
      })

      // Act: Render VideoCard
      const { container } = renderWithProviders(<VideoCard video={mockVideo} />)
    ...
    it('updates progress overlay as import progresses', async () => {
      ...
      act(() => {
        useImportProgressStore.getState().setProgress('video-import-123', 50, 'captions')
      })
      ...
      act(() => {
        useImportProgressStore.getState().setProgress('video-import-123', 75, 'chapters')
      })
    ...
    it('removes progress overlay when import completes', async () => {
      ...
      // Act: Complete import
      act(() => {
        useImportProgressStore.getState().setProgress('video-import-123', 100, 'complete')
      })
  ...
  describe('Phase 5: Error State Display', () => {
    ...
    it('shows error indicator for failed imports', () => {
      const { container } = renderWithProviders(<VideoCard video={errorVideo} />)
  ...
  describe('ProgressOverlay Component', () => {
    ...
    it('clamps progress between 0 and 100', () => {
      // Test over 100
      const { rerender } = render(<ProgressOverlay progress={150} stage="complete" />)
  ...
  describe('Import Progress Store', () => {
    it('tracks progress for multiple videos independently', () => {
      ...
      expect(store.getProgress('video-1')).toEqual({ progress: 25, stage: 'metadata' })
      expect(store.getProgress('video-2')).toEqual({ progress: 75, stage: 'chapters' })
    })
  ...
  describe('Full Import Flow Simulation', () => {
    ...
    it('simulates complete WebSocket progress flow', async () => {
      ...
      const { container, unmount } = renderWithProviders(<VideoCard video={mockVideo} />)
      ...
      act(() => {
        useImportProgressStore.getState().setProgress('video-full-flow', 25, 'metadata')
      })
      ...
      act(() => {
        useImportProgressStore.getState().setProgress('video-full-flow', 50, 'captions')
      })
      ...
      // Step 4: Complete
      act(() => {
        useImportProgressStore.getState().setProgress('video-full-flow', 100, 'complete')
      })
    ...
    it('simulates error flow', async () => {
      ...
      act(() => {
        useImportProgressStore.getState().setProgress('video-full-flow', 25, 'error')
      })
