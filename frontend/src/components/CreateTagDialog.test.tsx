...
describe("CreateTagDialog - Schema Selection", () => {
  ...
  it("shows Task #83 placeholder text when component is rendered", () => {
    ...
    expect(
      screen.queryByText(/Schema-Editor wird in Task #83 implementiert/i)
  ...
  it("validates tag name is required", async () => {
    ...
    renderWithProviders(
      <CreateTagDialog listId="list-1" onOpenChange={vi.fn()} open={true} />
    );
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Bitte geben Sie einen Tag-Namen ein/i)
      ).toBeInTheDocument();
    });
  });
