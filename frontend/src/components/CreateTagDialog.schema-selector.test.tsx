  ...
  queryFn: async () =>
    [
      ...
      },
      {
        id: "schema-2",
        name: "Content Rating",
        description: "Rate content quality",
        list_id: "list-123",
      },
...
const _createMockSchema = (
  overrides: Partial<FieldSchemaResponse> = {}
): FieldSchemaResponse => ({
  id: "schema-123",
  name: "Test Schema",
  description: null,
...
describe("CreateTagDialog - Schema Selector Extension", () => {
  ...
  const renderDialog = (
    props: Partial<React.ComponentProps<typeof CreateTagDialog>> = {}
  ) => {
    const defaultProps = {
      open: true,
      onOpenChange: vi.fn(),
  ...
  describe("Schema Selector Rendering", () => {
    ...
    it('shows "+ Neues Schema erstellen" as last option', async () => {
      ...
      const newSchemaOption = await screen.findByText(
