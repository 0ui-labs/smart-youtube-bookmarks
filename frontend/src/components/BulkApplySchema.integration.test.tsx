...
// Mock useTags hooks - but let useBulkApplySchema use real implementation
// Only mock tagsOptions to provide consistent query key
vi.mock("@/hooks/useTags", async () => {
  const actual =
    await vi.importActual<typeof import("@/hooks/useTags")>("@/hooks/useTags");
  return {
    ...actual,
    tagsOptions: vi.fn(() => ({
      queryKey: ["tags"],
...
describe("BulkApplySchema Integration", () => {
  ...
  it("completes full bulk apply flow successfully", async () => {
    ...
    render(<SchemaCard listId="list-1" schema={mockSchema} />, { wrapper });

    // 1. Open actions menu
    await user.click(
      screen.getByRole("button", { name: /actions for video quality/i })
    );
    ...
    await waitFor(() => {
      ...
      expect(api.put).toHaveBeenCalledWith("/tags/tag-1", {
        schema_id: "schema-1",
      });
      expect(api.put).toHaveBeenCalledWith("/tags/tag-2", {
        schema_id: "schema-1",
      });
    });
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(/2 von 2 tags erfolgreich aktualisiert/i)
      ).toBeInTheDocument();
    });
  });
  ...
  it("handles partial failure correctly", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Schema teilweise angewendet")
      ).toBeInTheDocument();
      expect(
        screen.getByText(/1 von 2 tags erfolgreich aktualisiert/i)
      ).toBeInTheDocument();
    ...
    expect(
      screen.getByRole("button", { name: /fehlgeschlagene wiederholen/i })
    ).toBeInTheDocument();
  });
  ...
  it("allows retrying failed operations", async () => {
    ...
    await waitFor(() => {
      expect(api.put).toHaveBeenCalledTimes(2);
      expect(api.put).toHaveBeenNthCalledWith(2, "/tags/tag-1", {
        schema_id: "schema-1",
      });
    });
