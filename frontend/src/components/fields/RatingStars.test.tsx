...
describe("RatingStars Component", () => {
  ...
  describe("Rendering", () => {
    ...
    it("displays filled stars based on current value", () => {
      const { container } = render(
        <RatingStars {...defaultProps} maxRating={5} value={4} />
      );
    ...
    it("displays empty stars for unset values", () => {
      const { container } = render(
        <RatingStars {...defaultProps} maxRating={5} value={2} />
      );
      const stars = container.querySelectorAll("svg");
    ...
    it("handles null value correctly", () => {
      const { container } = render(
        <RatingStars {...defaultProps} maxRating={5} value={null} />
      );
      const stars = container.querySelectorAll("svg");
      const emptyStars = Array.from(stars).filter((star) =>
        star.classList.contains("fill-none")
  ...
  describe("Keyboard Navigation", () => {
    it("increases rating on ArrowRight", async () => {
      const user = userEvent.setup();
      render(<RatingStars {...defaultProps} maxRating={5} value={3} />);
    ...
    it("prevents rating from going below 0", async () => {
      const user = userEvent.setup();
      render(<RatingStars {...defaultProps} maxRating={5} value={0} />);
    ...
    it("prevents rating from exceeding maxRating", async () => {
      const user = userEvent.setup();
      render(<RatingStars {...defaultProps} maxRating={5} value={5} />);
  ...
  describe("Read-only Mode", () => {
    ...
    it("does not trigger hover preview in read-only mode", async () => {
      const user = userEvent.setup();
      const { container } = render(
        <RatingStars {...defaultProps} maxRating={5} readonly value={2} />
      );
  ...
  describe("Size Variants", () => {
    it("applies correct size classes for sm", () => {
      const { container } = render(
        <RatingStars {...defaultProps} maxRating={3} size="sm" />
      );
      const stars = container.querySelectorAll("svg");
      stars.forEach((star) => {
    ...
    it("applies correct size classes for md", () => {
      const { container } = render(
        <RatingStars {...defaultProps} maxRating={3} size="md" />
      );
      const stars = container.querySelectorAll("svg");
      stars.forEach((star) => {
    ...
    it("applies correct size classes for lg", () => {
      const { container } = render(
        <RatingStars {...defaultProps} maxRating={3} size="lg" />
      );
      const stars = container.querySelectorAll("svg");
      stars.forEach((star) => {
  ...
  describe("Accessibility", () => {
    ...
    it("aria-label shows 0 for null value", () => {
      render(<RatingStars {...defaultProps} maxRating={5} value={null} />);
      const group = screen.getByRole("radiogroup");
    ...
    it("uses roving tabindex (only selected radio is focusable)", () => {
      render(<RatingStars {...defaultProps} maxRating={3} value={2} />);
      const buttons = screen.getAllByRole("radio");
    ...
    it("buttons are not focusable when readonly", () => {
      render(<RatingStars {...defaultProps} maxRating={3} readonly />);
      const buttons = screen.getAllByRole("radio");
      buttons.forEach((button) => {
  ...
  describe("Edge Cases", () => {
    it("handles maxRating of 1", () => {
      render(<RatingStars {...defaultProps} maxRating={1} value={1} />);
      const buttons = screen.getAllByRole("radio");
      expect(buttons).toHaveLength(1);
    });

    it("handles maxRating of 10", () => {
      render(<RatingStars {...defaultProps} maxRating={10} value={7} />);
      const buttons = screen.getAllByRole("radio");
      expect(buttons).toHaveLength(10);
    });
    ...
    it("handles value of 0", () => {
      const { container } = render(
        <RatingStars {...defaultProps} maxRating={5} value={0} />
      );
      const stars = container.querySelectorAll("svg");
      const filledStars = Array.from(stars).filter((star) =>
        star.classList.contains("fill-yellow-400")
