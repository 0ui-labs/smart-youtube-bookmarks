...
function TestWrapper({ defaultOptions = ['Option 1'] }: { defaultOptions?: string[] }) {
  const form = useForm({
    defaultValues: {
      config: {
...
describe('SelectConfigEditor', () => {
  ...
  describe('Rendering', () => {
    ...
    it('renders add option input and button', () => {
      ...
      expect(screen.getByPlaceholderText(/neue option hinzufügen/i)).toBeInTheDocument()
      expect(screen.getByRole('button', { name: /option hinzufügen/i })).toBeInTheDocument()
    })
  })
  ...
  describe('Adding Options', () => {
    it('adds new option when clicking add button', async () => {
      ...
      await user.click(screen.getByRole('button', { name: /option hinzufügen/i }))
    ...
    it('clears input after adding option', async () => {
      ...
      const input = screen.getByPlaceholderText(/neue option hinzufügen/i) as HTMLInputElement
  ...
  describe('Validation', () => {
    it('shows error when adding empty option', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/option darf nicht leer sein/i)).toBeInTheDocument()
      })
    })
    ...
    it('shows error when adding duplicate option (case-insensitive)', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/diese option existiert bereits/i)).toBeInTheDocument()
      })
    })
    ...
    it('clears error when typing after duplicate error', async () => {
      ...
      await waitFor(() => {
        expect(screen.queryByText(/diese option existiert bereits/i)).not.toBeInTheDocument()
      })
    })
  })
