      ...
      <div>
        ...
        <select
          ...
          onChange={(e) => {
            ...
            if (newType === 'select') {
              form.setValue('config', { options: [{ value: 'Default Option' }] })
...
describe('FieldConfigEditor - Integration Tests', () => {
  describe('Type Switching', () => {
    it('switches from select to rating config editor', async () => {
      ...
      expect(screen.queryByPlaceholderText(/neue option hinzufügen/i)).not.toBeInTheDocument()
    })
    ...
    it('switches from rating to text config editor', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByLabelText(/zeichenlimit festlegen/i)).toBeInTheDocument()
      })

      // Rating config should be gone
      expect(screen.queryByLabelText(/maximale bewertung/i)).not.toBeInTheDocument()
    })
  })
})
