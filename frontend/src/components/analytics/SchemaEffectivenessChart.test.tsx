...
describe('SchemaEffectivenessChart', () => {
  ...
  it('renders chart with data', () => {
    ...
    expect(screen.getByText(/Completion rate for \d+ schemas/)).toBeInTheDocument()
  })

  it('renders empty state when no data', () => {
    render(<SchemaEffectivenessChart data={[]} />)

    expect(screen.getByText('No schemas with field values yet')).toBeInTheDocument()
    expect(screen.getByText('Start filling in schema fields to see effectiveness metrics')).toBeInTheDocument()
  })

  it('displays correct number of schemas in description', () => {
    render(<SchemaEffectivenessChart data={mockData} />)

    expect(screen.getByText('Completion rate for 3 schemas')).toBeInTheDocument()
  })
  ...
  it('uses singular form for single schema', () => {
    ...
    expect(screen.getByText('Completion rate for 1 schema')).toBeInTheDocument()
  })
})
