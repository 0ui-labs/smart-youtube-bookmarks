 ...
 */
describe.skip('SchemaEditor Integration Tests', () => {
  ...
  it('creates schema with new field end-to-end', async () => {
    ...
    const fieldSelectorButton = screen.getByRole('button', { name: /vorhandene felder/i })
  ...
  it('enforces max 3 show_on_card limit', async () => {
    ...
    expect(screen.queryByText(/maximal 3 felder können auf der karte angezeigt werden/i)).not.toBeInTheDocument()
    ...
    await waitFor(() => {
      expect(screen.getByText(/maximal 3 felder können auf der karte angezeigt werden/i)).toBeInTheDocument()
    })
  ...
  it('prevents duplicate field names via FieldSelector', async () => {
    ...
    const selectorButton = screen.getByRole('button', { name: /1 feld ausgewählt/i })
    ...
    await waitFor(() => {
      ...
      const existingFieldOptions = screen.queryAllByText('Existing Field')
