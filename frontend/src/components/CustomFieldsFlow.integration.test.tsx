...
const _API_BASE = "http://localhost:8000/api";
...
describe("Custom Fields Flow Integration (Task #134)", () => {
    ...
    server.use(
      ...
      http.post("/api/lists/:listId/schemas", async ({ request }) => {
        ...
        const newSchema = createMockSchema({
          ...
          schema_fields:
            body.fields?.map((f: any, index: number) => ({
              field_id: f.field_id,
              schema_id: MOCK_SCHEMA_ID,
              display_order: f.display_order ?? index,
              show_on_card: f.show_on_card ?? false,
              field:
                mockFields.find((field) => field.id === f.field_id) ||
                createMockField({ id: f.field_id }),
            })) || [],
        });
      ...
      http.post(
        "/api/lists/:listId/custom-fields/check-duplicate",
        async ({ request }) => {
          const body = (await request.json()) as any;
      ...
      http.put("/api/videos/:videoId/fields", async ({ request, params }) => {
        ...
        if (video) {
          video.field_values = body.field_values.map((fv: any) => ({
            ...
            field:
              mockFields.find((f) => f.id === fv.field_id) ||
              createMockField({ id: fv.field_id }),
            value: fv.value,
            show_on_card: true,
  ...
  describe("Test 1: Create tag with new schema and custom field", () => {
    it("creates tag with schema containing rating field (outcome-based)", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("How to Apply Perfect Eyeliner")
        ).toBeInTheDocument();
      });
      ...
      // STEP 5: Save tag
      const saveTagButton = screen.getByRole("button", {
  ...
  describe("Test 2: Assign tag to video and set field value", () => {
    ...
    it("assigns tag to video and sets rating field value to 5 stars", async () => {
      ...
      await waitFor(() => {
        expect(
          within(videoCard).getByText("Makeup Tutorials")
        ).toBeInTheDocument();
      });
      ...
      // Click 5th star
      const stars = within(videoCard).getAllByRole("button", {
        name: /stern|star/i,
      });
  ...
  describe("Test 3: Error handling for API failures", () => {
    it("shows error toast when field creation fails", async () => {
      ...
      const saveFieldButton = screen.getByRole("button", {
        name: /feld.*speichern/i,
      });
