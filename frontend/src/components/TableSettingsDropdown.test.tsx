...
describe("TableSettingsDropdown", () => {
  ...
  describe("Thumbnail Size Selection", () => {
    it("shows thumbnail size options when opened", async () => {
      ...
      expect(
        screen.getByRole("menuitemradio", { name: /klein/i })
      ).toBeInTheDocument();
      expect(
        screen.getByRole("menuitemradio", { name: /mittel/i })
      ).toBeInTheDocument();
      expect(
        screen.getByRole("menuitemradio", { name: /groß/i })
      ).toBeInTheDocument();
    });
  ...
  describe("Column Visibility", () => {
    it("shows column visibility checkboxes when opened", async () => {
      ...
      expect(
        screen.getByRole("menuitemcheckbox", { name: /thumbnail/i })
      ).toBeInTheDocument();
      expect(
        screen.getByRole("menuitemcheckbox", { name: /titel/i })
      ).toBeInTheDocument();
      expect(
        screen.getByRole("menuitemcheckbox", { name: /dauer/i })
      ).toBeInTheDocument();
      expect(
        screen.getByRole("menuitemcheckbox", { name: /aktionen/i })
      ).toBeInTheDocument();
    });
    ...
    it("shows current column visibility state", async () => {
      ...
      const titleCheckbox = screen.getByRole("menuitemcheckbox", {
        name: /titel/i,
      });
      expect(titleCheckbox).toHaveAttribute("aria-checked", "false");

      // Verify thumbnail checkbox is checked
      const thumbnailCheckbox = screen.getByRole("menuitemcheckbox", {
        name: /thumbnail/i,
      });
  ...
  describe("Grid Column Control (Task #34)", () => {
    ...
    it("shows grid column control when viewMode is grid", async () => {
      ...
      expect(
        screen.getByRole("menuitemradio", { name: /2 Spalten \(Breit\)/i })
      ).toBeInTheDocument();
      expect(
        screen.getByRole("menuitemradio", { name: /3 Spalten \(Standard\)/i })
      ).toBeInTheDocument();
      expect(
        screen.getByRole("menuitemradio", { name: /4 Spalten \(Kompakt\)/i })
      ).toBeInTheDocument();
      expect(
        screen.getByRole("menuitemradio", { name: /5 Spalten \(Dicht\)/i })
      ).toBeInTheDocument();
    });
    ...
    it("calls setGridColumns with number when option clicked", async () => {
      ...
      const option5 = screen.getByRole("menuitemradio", {
        name: /5 Spalten \(Dicht\)/i,
      });
    ...
    it("shows current gridColumns value as checked", async () => {
      ...
      const option4 = screen.getByRole("menuitemradio", {
        name: /4 Spalten \(Kompakt\)/i,
      });
    ...
    it("has accessible aria-label on RadioGroup", async () => {
      ...
      const radioGroup = screen.getByRole("group", {
        name: /spaltenanzahl für grid-ansicht/i,
      });
  ...
  describe("Video Details View (Task #131)", () => {
    it('renders Video Details RadioGroup with correct default ("page")', async () => {
      ...
      expect(
        screen.getByLabelText("Eigene Seite (Standard)")
      ).toBeInTheDocument();
