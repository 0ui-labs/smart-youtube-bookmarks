...
describe("VideosPage - Feature Flags (Task #24)", () => {
  ...
  describe("Component Rendering", () => {
    ...
    it("renders the videos page title", () => {
      ...
      expect(
        screen.getByRole("heading", { name: /Videos/i, level: 1 })
      ).toBeInTheDocument();
    });
  });
...
describe("VideosPage - Sorting (Task #146 Section 7)", () => {
  ...
  describe("Column Header Click Updates URL", () => {
    it("updates URL when clicking on Titel header for ascending sort", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByLabelText("Aufsteigend sortiert")
        ).toBeInTheDocument();
      });
    });
    ...
    it("updates URL when clicking on Dauer header for ascending sort", async () => {
      ...
      const durationHeaders = screen
        .getAllByRole("button")
        .filter((btn) => btn.textContent?.includes("Dauer"));
  ...
  describe("Sort Direction Toggle", () => {
    it("toggles sort direction on second click of same column", async () => {
      ...
      const titleHeaders = screen
        .getAllByRole("button")
        .filter((btn) => btn.textContent?.includes("Titel"));
      ...
      await waitFor(
        () => {
          expect(
            screen.getByLabelText("Absteigend sortiert")
          ).toBeInTheDocument();
        },
        { timeout: 3000 }
      );
  ...
  describe("Sort Indicator Display", () => {
    ...
    it("does not show sort indicator when no sort is active", async () => {
      ...
      expect(
        screen.queryByLabelText("Aufsteigend sortiert")
      ).not.toBeInTheDocument();
      expect(
        screen.queryByLabelText("Absteigend sortiert")
      ).not.toBeInTheDocument();
    });
  });
});
